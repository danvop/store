            ...
            <x-dropdown>
                <x-slot name="trigger">
                    ...
                    <button
                        ...
                        >
                        {{ isset($currentStore) ? ucwords($currentStore->name) : 'Store' }}
                ...
                @foreach ($stores as $store)
                    <a href="/stores/{{ $store->id }}" class="block text-left px-3 text-sm leading-6 hover:bg-blue-500 focus:bg-blue-500 hover:text-white focus:text-white
                        {{ isset($currentStore) && $currentStore->is($store) ? 'bg-blue-500 text-white' : '' }}
                        "
                    >{{ ucwords($store->name) }}</a>
                @endforeach
            </x-dropdown>
